...
const DroneInfo = ({ selectedDrone, selectedTime }) => {
  const droneColor = "#000000";
  const [showCesiumMap, setShowCesiumMap] = useState(false); // Initially set to false to show the button first

  const handleClick = () => {
    setShowCesiumMap(!showCesiumMap); // Toggle the visibility of the CesiumMap
  };
    ...
    <div style={{ width: '20%', padding: '10px', backgroundColor: 'white', borderLeft: '1px solid #ccc' }}>
      ...
      {showCesiumMap && (
        <div style={{
          ...
          <CesiumMap droneData={selectedDrone} droneColor={droneColor} />
        </div>
      )}
    </div>
  );
